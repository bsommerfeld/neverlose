...
import de.bsommerfeld.neverlose.fx.viewmodel.ExerciseViewModel;
import de.bsommerfeld.neverlose.fx.viewmodel.TrainingPlanEditorViewModel;
import de.bsommerfeld.neverlose.fx.viewmodel.TrainingUnitViewModel;
...
public class TrainingPlanEditorFormController {
  ...
  @FXML private TextField planNameTextField;
  @FXML private TextArea planDescriptionTextArea;
  ...
  @FXML private VBox unitListContainer;
  @FXML private ListView<TrainingUnitViewModel> trainingUnitsListView;
  @FXML private Button addUnitButton;

  @FXML private VBox unitDetailContainer;
  @FXML private TextField unitNameTextField;
  @FXML private TextArea unitDescriptionTextArea;
  @FXML private ChoiceBox<Weekday> unitWeekdayChoiceBox;
  @FXML private ListView<ExerciseViewModel> trainingExercisesListView;
  @FXML private Button addExerciseButton;

  @FXML private VBox exerciseDetailContainer;
  @FXML private TextField exerciseNameTextField;
  @FXML private TextArea exerciseDescriptionTextArea;
  @FXML private TextField exerciseDurationTextField;
  @FXML private TextField exerciseSetsTextField; // Maybe Spinner<Integer>
  @FXML private CheckBox exerciseBallBucketCheckBox;
  ...
  /**
   * Initializes the cell factories for the training units and exercises ListViews. Uses a helper
   * method {@link #createRemovableCell(Function, Consumer)} to configure cells with a remove button
   * and avoid code duplication.
   */
  private void initializeCellFactories() {
    trainingUnitsListView.setCellFactory(
        lv ->
            createRemovableCell(
                unitVm ->
                    (unitVm != null && unitVm.getModel() != null)
                        ? unitVm.getModel().getName()
                        : "Unit (no model)",
                unitVm -> {
                  if (unitVm != null && viewModel != null) viewModel.removeTrainingUnit(unitVm);
                }));
    trainingExercisesListView.setCellFactory(
        lv ->
            createRemovableCell(
                exerciseVm ->
                    (exerciseVm != null && exerciseVm.getModel() != null)
                        ? exerciseVm.getModel().getName()
                        : "Exercise (no model)",
                exerciseVm -> {
                  if (exerciseVm != null && viewModel != null)
                    viewModel.removeExerciseFromSelectedUnit(exerciseVm);
                }));
  }

  /**
   * Creates a generic ListCell instance configured to display an item's name alongside a "Remove"
   * button. The cell includes a flexible spacer to push the button to the right end of the cell.
   *
   * @param <T> The type of the item displayed in the ListView cell.
   * @param nameExtractor A {@link Function} that takes an item of type T and returns the String
   *     representation (e.g., name) to be displayed.
   * @param removeAction A {@link Consumer} that takes an item of type T and performs the action
   *     required to remove that item (typically involves calling a ViewModel method).
   * @return A configured {@link ListCell} instance for items of type T.
   */
  private <T> ListCell<T> createRemovableCell(
      Function<T, String> nameExtractor, Consumer<T> removeAction) {
    return new ListCell<>() {
      ...
      /**
       * Updates the cell's appearance based on the item it currently represents. Called by the
       * ListView virtualization mechanism.
       */
      @Override
  ...
  private void unbindUI() {
    ...
    TrainingUnitViewModel lastSelectedUnit =
        trainingUnitsListView.getSelectionModel().getSelectedItem();
    if (lastSelectedUnit != null) {
      ExerciseViewModel lastSelectedExercise =
          trainingExercisesListView.getSelectionModel().getSelectedItem();
  ...
  private void bindUI() {
    ...
    viewModel
        .selectedTrainingUnitProperty()
        .bind(trainingUnitsListView.getSelectionModel().selectedItemProperty());
    ...
    if (initialUnit != null) {
      ...
      viewModel
          .selectedTrainingUnitProperty()
          .bind(trainingUnitsListView.getSelectionModel().selectedItemProperty()); // Rebind
  ...
  /**
   * Makes the specified view node visible and managed within the StackPane, hiding and unmanaging
   * the others. Also resets scroll position.
   *
  ...
  private void updateBreadcrumb(
      TrainingUnitViewModel selectedUnit, ExerciseViewModel selectedExercise) {
    breadcrumbBox.getChildren().clear();
  ...
  private void unbindExerciseDetails(ExerciseViewModel exerciseVm) {
    ...
    exerciseDescriptionTextArea
        .textProperty()
        .unbindBidirectional(exerciseVm.descriptionProperty());
    exerciseDurationTextField.textProperty().unbindBidirectional(exerciseVm.durationProperty());
    exerciseBallBucketCheckBox
        .selectedProperty()
        .unbindBidirectional(exerciseVm.ballBucketProperty());
    unbindSetsTextField();
  }

  private void onSelectedUnitChanged(
      TrainingUnitViewModel oldSelectedUnit, TrainingUnitViewModel newSelectedUnit) {
    log.info(
        "Selected Unit Changed - New: "
            + (newSelectedUnit != null ? newSelectedUnit.nameProperty().get() : "null"));
    ...
    if (newSelectedUnit != null) {
      ...
      unitDescriptionTextArea
          .textProperty()
          .bindBidirectional(newSelectedUnit.descriptionProperty());
      ...
      newSelectedUnit
          .selectedExerciseProperty()
          .bind(trainingExercisesListView.getSelectionModel().selectedItemProperty());
  ...
  private void onSelectedExerciseChanged(
      ExerciseViewModel oldSelectedExercise, ExerciseViewModel newSelectedExercise) {
    log.info(
        "Selected Exercise Changed - New: "
            + (newSelectedExercise != null ? newSelectedExercise.nameProperty().get() : "null"));
    TrainingUnitViewModel currentUnit = viewModel.selectedTrainingUnitProperty().get();
    ...
    if (newSelectedExercise != null) {
      ...
      exerciseDescriptionTextArea
          .textProperty()
          .bindBidirectional(newSelectedExercise.descriptionProperty());
      exerciseDurationTextField
          .textProperty()
          .bindBidirectional(newSelectedExercise.durationProperty());
      exerciseBallBucketCheckBox
          .selectedProperty()
          .bindBidirectional(newSelectedExercise.ballBucketProperty());
    ...
    vmToTextSetsListener =
        (obs, oldVal, newVal) -> {
          ...
          try {
            ...
            /* Ignore, comparison below handles it */
          }
  ...
  private void handleExportPdfAction() {
    ...
    if (selectedFile != null) {
      ...
      try {
        ...
        showConfirmation(
            "Export Successful",
            "Training plan exported successfully to:\n" + selectedFile.getName());
      } catch (IOException e) {
        System.err.println("Error exporting PDF: " + e.getMessage());
        e.printStackTrace();
        showError(
            "Export Failed", "Could not export training plan to PDF.\nError: " + e.getMessage());
      } catch (Exception e) {
  ...
  /** f Cleans up listeners when the view is potentially being destroyed. */
  public void cleanup() {
