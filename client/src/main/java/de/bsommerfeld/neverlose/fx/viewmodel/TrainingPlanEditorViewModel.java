...
/**
 * ViewModel for the Training Plan Editor view. Manages the state of the editor, including the plan
 * details and its units, and provides properties for UI binding and methods for actions like
 * adding/removing units/exercises and exporting the plan. It handles the synchronization between
 * the ViewModel state and the underlying TrainingPlan model.
 */
public class TrainingPlanEditorViewModel {
  ...
  private final ObservableList<TrainingUnitViewModel> trainingUnits =
      FXCollections.observableArrayList();
  private final ObjectProperty<TrainingUnitViewModel> selectedTrainingUnit =
  ...
  private final ListChangeListener<TrainingUnitViewModel> trainingUnitsModelUpdater =
      c -> {
        if (trainingPlan == null || trainingPlan.getTrainingUnits() == null) {
          // System.err.println("Warning: TrainingPlan or its TrainingUnits collection is null. Sync
          // disabled.");
          return;
        }
        while (c.next()) {
  ...
  /**
   * Loads a new TrainingPlan into the ViewModel using a robust reset strategy. Removes listeners,
   * explicitly clears the main list to reset UI state, sets the new model, updates properties,
   * repopulates the list, and re-adds listeners.
   *
  ...
  public void loadPlan(TrainingPlan planToLoad) {
    ...
    List<TrainingUnit> modelUnits =
        trainingPlan.getTrainingUnits() != null
            ? trainingPlan.getTrainingUnits().getAll()
            : new ArrayList<>();

    List<TrainingUnitViewModel> newUnitViewModels =
        modelUnits.stream().map(TrainingUnitViewModel::new).collect(Collectors.toList());
  ...
  /**
   * Sets the TrainingPlan to be edited by this ViewModel. Delegates to {@link
  ...
  /**
   * Initiates the process to export the current training plan to a PDF file using the {@link
   * PdfExportService}.
   */
