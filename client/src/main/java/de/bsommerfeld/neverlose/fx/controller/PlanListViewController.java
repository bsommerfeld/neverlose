...
public class PlanListViewController {
  ...
  @FXML private FlowPane planFlowPane;
  private FilteredList<PlanSummary> filteredPlans;

  @Inject
  public PlanListViewController(
      PlanStorageService planStorageService, SearchState searchState, ViewProvider viewProvider) {
  ...
  private void initialize() {
    ...
    filteredPlans.addListener(
        (ListChangeListener<PlanSummary>)
            c -> {
  ...
  private void loadAndDisplayPlans() {
    ...
    try {
      ...
      log.error(
          "Failed to load plan summaries from storage path: {}",
          planStorageService.getStoragePath(),
          e);
  ...
  /**
   * Creates a simple UI node (VBox with Label) to represent a PlanSummary. Adds a click handler to
   * navigate to the editor for that plan.
   */
      ...
      Image icon =
          new Image(
              Objects.requireNonNull(
                  getClass().getResourceAsStream("/icons/plan-icon.jpg"))); // TODO: png
        ...
        new Label(
            "ID: "
                + summary
                    .identifier()
                    .toString()
                    .substring(0, Math.min(summary.identifier().toString().length(), 8))
                + "...");
  ...
  /**
   * Handles the selection of a plan from the list view. Loads the full plan and triggers a view
    ...
    try {
      ...
      if (planOptional.isPresent()) {
        ...
        viewProvider.triggerViewChange(
            TrainingPlanEditorMetaController.class, editor -> editor.setPlan(loadedPlan));
        searchState.setSearchTerm("");
      } else {
        log.error(
            "Failed to load selected plan with identifier: {}. File might be missing or corrupted.",
            summary.identifier());
