...
public class TrainingPlanEditorPreviewController {
  ...
  private final Map<ObservableList<?>, ListChangeListener<?>> previewListChangeListeners =
      new HashMap<>();
  private TrainingPlanEditorViewModel viewModel;

  @FXML private VBox previewVBox;
  @FXML private Label previewPlanNameLabel;
  @FXML private Text previewPlanDescriptionText;
  @FXML private Label unitsPreviewLabel;
  @FXML private VBox previewUnitsContainer;
  ...
  /**
   * Injects the ViewModel and triggers UI binding/listener setup. This method should be called by
   * the MetaController after FXML loading.
   *
  ...
  private void bindPreviewUI() {
    ...
    unitsPreviewLabel
        .visibleProperty()
        .bind(Bindings.isEmpty(previewUnitsContainer.getChildren()).not());
  ...
  /**
   * Sets up the listeners needed for the preview pane to update dynamically when the ViewModel's
   * data changes.
   */
  ...
  private VBox createUnitPreviewNode(TrainingUnitViewModel unitVm) {
    ...
    Label unitHeader =
        new Label(unitVm.getModel().getName() + " - " + unitVm.getModel().getWeekday());
  ...
  /**
   * Call this method when the view associated with this controller is being closed or disposed of
   * to prevent memory leaks by removing all listeners. Should be called by the MetaController.
   */
  ...
  private <T> void removePreviewListChangeListener(ObservableList<T> list) {
    if (list != null && previewListChangeListeners.containsKey(list)) {
      @SuppressWarnings("unchecked")
      ListChangeListener<T> listener =
          (ListChangeListener<T>) previewListChangeListeners.remove(list);
